...
//  <div className="p-4 shadow-lg rounded-lg flex-grow flex-shrink w-full bg-slate-100 text-black mt-4">
//       <h2 className="text-lg md:text-xl font-bold mb-4 text-green-500">Saved Templates</h2>
//       {templates.map((template) => (
//         <div key={template.id} className="mb-4 p-2 border-b-2">
//           <h3 className="font-semibold text-black display-flex text-base md:text-lg bg-blend-hue">{`${template.day}-${template.muscle}`}</h3>
//           <p className="text-sm text-orange-500">Day: {template.day}</p>
//           <p className="text-sm">Target Muscle: {template.muscle}</p>
//           <div>
//             <h4 className="font-semibold mt-2 text-sm md:text-base">Exercises:</h4>
...
//                 <li key={index} className="flex items-center justify-between mt-1">
//                   <span className="text-sm md:text-base">{exercise.name}</span>
//                   {exercise.gifpath && <img src={exercise.gifpath} alt={exercise.name} className="ml-2 w-12 h-12 md:w-16 md:h-16 rounded" loading="lazy" />}
//                 </li>
//               ))}
//             </ul>
//           </div>
//           <button
//             onClick={() => deleteTemplate(template.id)} // Assuming delete by ID now
